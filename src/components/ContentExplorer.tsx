...
export default function ContentExplorer({ initialType = "all" }: { initialType?: Tab }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 lg:px-8 text-slate-800 relative z-10">
      {/* Inline Content-Specific Robotic Telemetry Panels (Behinds the Cards & Filters) */}
      <div className="absolute inset-0 pointer-events-none -z-10 overflow-hidden rounded-3xl select-none">
        
        {/* Upper Left Header Accent (Directly behind the title area) */}
        <svg className="absolute left-4 top-4 w-64 h-36 opacity-15 text-slate-800" viewBox="0 0 200 120" fill="none" stroke="currentColor" strokeWidth="1.2">
          <path d="M 0 10 H 120 L 140 30 V 80 H 180" />
          <path d="M 20 30 H 100 L 110 40" strokeDasharray="3,3" />
          <circle cx="120" cy="10" r="2.5" fill="currentColor" />
          <circle cx="140" cy="30" r="2.5" fill="currentColor" />
          <text x="10" y="25" fontSize="6" fontFamily="monospace" className="fill-current font-black">SYS.PORTAL_CORE</text>
        </svg>

        {/* Upper Right Header Accent */}
        <svg className="absolute right-4 top-4 w-64 h-36 opacity-15 text-slate-800" viewBox="0 0 200 120" fill="none" stroke="currentColor" strokeWidth="1.2">
          <path d="M 200 10 H 80 L 60 30 V 80 H 20" />
          <circle cx="80" cy="10" r="2.5" fill="currentColor" />
          <circle cx="60" cy="30" r="2.5" fill="currentColor" />
          <text x="110" y="25" fontSize="6" fontFamily="monospace" className="fill-current font-black text-right">SYS.BAUD_9600</text>
        </svg>

        {/* AELOS Humanoid Robot Blueprint backing the cards */}
        <svg className="absolute left-1/2 top-[45%] -translate-x-1/2 -translate-y-1/2 w-[90%] h-[90%] opacity-[0.14] text-slate-800/85" viewBox="0 0 1000 800" fill="none" stroke="currentColor" strokeWidth="1.2">
          {/* Grid Background accents */}
          <path d="M 100 100 H 900 V 700 H 100 Z" strokeDasharray="4,8" />
          <line x1="500" y1="50" x2="500" y2="750" strokeDasharray="8,8" strokeWidth="1.5" />
          <line x1="100" y1="400" x2="900" y2="400" strokeDasharray="8,8" />

          {/* ================= ROBOT SCHEMA DRAWING ================= */}
          
          {/* Head (Sleek humanoid head) */}
          <circle cx="500" cy="200" r="28" strokeWidth="1.8" />
          <rect x="482" y="190" width="36" height="6" rx="2.5" fill="currentColor" /> {/* Visor */}
          <path d="M 475 200 H 525" />
          <path d="M 500 172 V 162" /> {/* Antenna */}
          <circle cx="500" cy="162" r="2" fill="currentColor" />

          {/* Neck */}
          <rect x="492" y="228" width="16" height="15" strokeWidth="1.5" />
          <line x1="488" y1="235" x2="512" y2="235" />

          {/* Torso / Chest Plates */}
          <rect x="440" y="243" width="120" height="160" rx="16" strokeWidth="2" />
          <path d="M 440 280 H 560" />
          <path d="M 440 330 H 560" strokeDasharray="3,3" />
          {/* Core Power Ring (Central chest circle) */}
          <circle cx="500" cy="305" r="20" strokeWidth="1.5" />
          <circle cx="500" cy="305" r="10" strokeDasharray="2,2" />
          <circle cx="500" cy="305" r="3" fill="currentColor" />

          {/* Shoulders & Arms */}
          {/* Left Shoulder Servo */}
          <circle cx="418" cy="265" r="12" strokeWidth="1.5" />
          <path d="M 406 265 H 340 V 360 L 310 400" strokeWidth="1.5" />
          {/* Left Hand claw */}
          <circle cx="310" cy="400" r="5" fill="currentColor" />
          <path d="M 302 392 Q 310 395 310 400 Q 310 405 302 408" strokeWidth="1.5" />

          {/* Right Shoulder Servo */}
          <circle cx="582" cy="265" r="12" strokeWidth="1.5" />
          <path d="M 594 265 H 660 V 360 L 690 400" strokeWidth="1.5" />
          {/* Right Hand claw */}
          <circle cx="690" cy="400" r="5" fill="currentColor" />
          <path d="M 698 392 Q 690 395 690 400 Q 690 405 698 408" strokeWidth="1.5" />

          {/* Pelvis / Hips */}
          <rect x="450" y="403" width="100" height="35" rx="6" strokeWidth="1.8" />
          <circle cx="475" cy="420" r="10" strokeWidth="1.5" />
          <circle cx="525" cy="420" r="10" strokeWidth="1.5" />

          {/* Legs */}
          {/* Left Leg Linkage */}
          <path d="M 475 430 V 540 L 450 640 H 420" strokeWidth="1.5" />
          <circle cx="475" cy="540" r="8" strokeWidth="1.5" /> {/* Knee joint */}
          <circle cx="450" cy="640" r="6" strokeWidth="1.5" /> {/* Ankle joint */}
          <rect x="410" y="640" width="30" height="8" rx="2" strokeWidth="1.5" /> {/* Foot */}

          {/* Right Leg Linkage */}
          <path d="M 525 430 V 540 L 550 640 H 580" strokeWidth="1.5" />
          <circle cx="525" cy="540" r="8" strokeWidth="1.5" /> {/* Knee joint */}
          <circle cx="550" cy="640" r="6" strokeWidth="1.5" /> {/* Ankle joint */}
          <rect x="560" y="640" width="30" height="8" rx="2" strokeWidth="1.5" /> {/* Foot */}

          {/* ================= TELEMETRY LEADERS & NOTES ================= */}
          
          {/* Head callout */}
          <path d="M 528 200 H 630 L 650 215" strokeWidth="1" />
          <text x="640" y="225" fontSize="7" fontFamily="monospace" className="fill-current font-bold">AELOS_H17.CAM_HD</text>
          
          {/* Left Shoulder callout */}
          <path d="M 406 265 H 250 L 230 250" strokeWidth="1" />
          <text x="140" y="245" fontSize="7" fontFamily="monospace" className="fill-current font-bold">SHOULDER_SERVO.L1</text>

          {/* Chest Core callout */}
          <path d="M 500 325 V 365 H 370" strokeWidth="1" strokeDasharray="2,2" />
          <text x="290" y="362" fontSize="7" fontFamily="monospace" className="fill-current font-bold">CORE_MCU.ATMEGA</text>

          {/* Right Knee callout */}
          <path d="M 533 540 H 640 L 660 560" strokeWidth="1" />
          <text x="670" y="570" fontSize="7" fontFamily="monospace" className="fill-current font-bold">KNEE_ACTUATOR.R12</text>

          {/* Technical Specs Overlay */}
          <text x="120" y="130" fontSize="8" fontFamily="monospace" className="fill-current font-black">MODEL: AELOS-HUMANOID v4.2</text>
          <text x="120" y="145" fontSize="7" fontFamily="monospace" className="fill-current">DEGREES OF FREEDOM: 17 DOFs</text>
          <text x="120" y="160" fontSize="7" fontFamily="monospace" className="fill-current">SERVO BUS: COAXIAL HALF-DUPLEX</text>
          <text x="120" y="175" fontSize="7" fontFamily="monospace" className="fill-current">SYS_VOLT: 7.4V DC</text>

          <text x="730" y="650" fontSize="8" fontFamily="monospace" className="fill-current font-black">KINEMATICS: OK</text>
          <text x="730" y="665" fontSize="7" fontFamily="monospace" className="fill-current">HEAD PAN: -90° TO +90°</text>
          <text x="730" y="680" fontSize="7" fontFamily="monospace" className="fill-current">SERVO TEMPS: 38.5°C</text>
        </svg>

        {/* Vertical side telemetry status panels */}
        <div className="absolute left-2 top-[35%] flex flex-col gap-6 font-mono text-[7px] text-slate-700 opacity-40 uppercase tracking-widest leading-none">
          <div>BUS STATUS: 100%</div>
          <div>CPU REG: 0x3F</div>
          <div>MEM LOC: 0xFF2A</div>
        </div>

        <div className="absolute right-2 top-[35%] flex flex-col gap-6 font-mono text-[7px] text-slate-700 opacity-40 uppercase tracking-widest leading-none text-right">
          <div>DATA STRM: V4</div>
          <div>FRAME RATE: 60</div>
          <div>CACHE REG: OK</div>
        </div>
      </div>
      {/* Hero */}

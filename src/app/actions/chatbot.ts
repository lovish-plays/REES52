...
export async function askAelosAiAction(prompt: string, history: GeminiHistoryItem[]) {
  ...
  const systemInstruction = 
    ...
    "CRITICAL DIRECTIVE: You must ONLY answer questions directly related to this domain (robotics, Arduino, sensors, drone DIY, electronics, engineering, REES52 kits, ebooks, video lectures, and the Infinity Learning Hub). " +
    ...
    "Infinity Learning Hub when asked about those topics. You are speaking directly to a user in a chat window.";
  ...
  try {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "X-Title": "REES52 Infinity Learning Hub"
      },
      body: JSON.stringify({
        model,
        messages,

...
export async function askAelosAiAction(prompt: string, history: GeminiHistoryItem[]) {
  ...
  if (!cleanPrompt) {
    return { success: false, error: 'Please enter a question for AELOS.' };
  }
  if (cleanPrompt.length > 1000) {
    return { success: false, error: 'Your question must be under 1000 characters.' };
  }
  ...
  if (!apiKey) {
    ...
    return { success: false, error: "AI Chatbot service is temporarily unavailable." };
  }

  const sanitizedHistory = Array.isArray(history)
    ? history.slice(-6).map((h) => ({
        role: h.role === 'user' ? ('user' as const) : ('assistant' as const),
        content: (h.parts || '').substring(0, 1000),
      }))
    : [];
  ...
  try {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer": process.env.NEXT_PUBLIC_SITE_URL || "https://rees52.tech",
    ...
    if (!response.ok) {
      ...
      return { success: false, error: "The AI service is experiencing high load. Please try again in a moment." };
    }
    ...
    if (!botText) {
      return { success: false, error: "No response generated by the AI assistant." };
    }

    return { success: true, text: botText };
  } catch (error: any) {
    console.error("[OPENROUTER API EXCEPTION]", error);
    return { success: false, error: "Failed to connect to the AI assistant service." };
  }
}
